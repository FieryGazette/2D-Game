...
View_3D_Game::View_3D_Game(Model* model, unsigned short console_width, unsigned short console_height, MODE mode) :
...
void View_3D_Game::RenderCollideBox()
{
	for(vector<Object*>::iterator it = model->getObject()->begin(); it != model->getObject()->end(); ++it)
	{
		...
		modelStack.Translate(o->getBbox()->position.x, o->getBbox()->position.y, o->getBbox()->position.z);
